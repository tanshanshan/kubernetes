	...
	for _, test := range tests {
		...
		for _, p := range runningPods {
			if err := waitForPodToSchedule(cs, p); err != nil {
				t.Fatalf("Pod %v/%v didn't get scheduled: %v", p.Namepsace, p.Name, err)
			}
		}
		// Create pending pods.
		...
		for _, p := range pendingPods {
			if err := wait.Poll(100*time.Millisecond, wait.ForeverTestTimeout,
				...
				t.Errorf("Pod %v/%v didn't get marked unschedulable: %v", p.Namepsace, p.Name, err)
			}
		}
		...
		if err := waitForNominatedNodeName(cs, preemptor); err != nil {
			t.Errorf("Test [%v]: NominatedNodeName annotation was not set for pod %v/%v: %v", test.description, preemptor.Namepsace, preemptor.Name, err)
		}
